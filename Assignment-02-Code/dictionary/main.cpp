...
void looper() {
    ...
    string token, input, response, lineToken;
...
//        response = queryDictionary(dictionaryMap, tokens[0], tokens[1], tokens[2]);
//
//        cout << response << endl;
    }
};
...
map<string, vector<string>> createHash(const vector<string>& dataVector) {
    ...
    for (string currentValue : dataVector) {
